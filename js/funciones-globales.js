...
function validarCBU(sCBU){
    var result = true;
    var bloquesCBU = sCBU.split(" ");
    var bloque1 = bloquesCBU[0].split('');
    var bloque2 = bloquesCBU[1].split('');
    
    var bloque1Verif = '7139713';
    var bloque2Verif = '3971397139713';
    bloque1Verif = bloque1Verif.split('');
    bloque2Verif = bloque2Verif.split('');
    
    var suma1 = 0;
    for (var i = 0; i<=6; i++){
        suma1 += bloque1[i] * bloque1Verif[i];
    }
    var digito1 = 10 - (suma1 % 10);
    if (digito1 === 10){
        digito1 = 0;
    }
    if (digito1 == bloque1[7]){
        var suma2 = 0;
        for (var i = 0; i<=12; i++){
            suma2 += bloque2[i] * bloque2Verif[i];
        }
        var digito2 = 10 - (suma2 % 10);
        if (digito2 === 10){
            digito2 = 0;
        }
        if (digito2 != bloque2[13]){
            result = false;
        }
    } else {
        result = false;
...
function cuitSinGuiones(cuit){
    if (cuit){
        var resultado = cuit.substring(0,2) + cuit.substring(3,11) + cuit.substring(12,13);
        return  resultado;
    } else {
        return null;
    }
