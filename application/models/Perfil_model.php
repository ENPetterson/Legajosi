...
class Perfil_model extends CI_Model{
    ...
    public function savePerfil(){
        $perfil = R::load('perfil', $this->id);
        $perfil->nombre = $this->nombre;
        $perfil->apellido = $this->apellido;
        $perfil->esSoltero = $this->esSoltero;
        $perfil->color = $this->color;
        $perfil->comida = $this->comida;
        $perfil->musica = $this->musica;
        $perfil->pelicula = $this->pelicula;
        $perfil->esDeportista = $this->esDeportista;
        $perfil->esVegetariano = $this->esVegetariano;                                                        
    ...
    public function getPerfil(){
        $perfil = R::load('perfil', $this->id);
        return $perfil->export();
    }
    
    public function getColor(){
        $color = R::load('color', $this->id);
        return $color->export();
    }
    
    public function getComida(){
        $comida = R::load('comida', $this->id);
        return $comida->export();
    }

    public function getMusica(){
        $musica = R::load('musica', $this->id);
        return $musica->export();
    }

    public function getPelicula(){
        $pelicula = R::load('pelicula', $this->id);
        return $pelicula->export();
    }

    public function getPerfilesUsuario(){
        $perfiles = R::getCol('select perfil_id from perfil_usuario where usuario_id = ?', array($this->usuario_id));
        return $perfiles;
    }

    public function getPerfiles(){
        $perfiles = R::getAll('SELECT p.id, p.nombre, p.apellido, p.esSoltero, c.descripcion AS color, ca.descripcion AS comida, m.descripcion AS musica, pe.descripcion AS pelicula, p.esDeportista, p.esVegetariano
            FROM perfil p 
            INNER JOIN color c ON p.color = c.id
            INNER JOIN comida ca ON p.comida = ca.id
            INNER JOIN musica m ON p.musica = m.id
            INNER JOIN pelicula pe ON p.pelicula = pe.id 
            ORDER BY p.id');
        return $perfiles;
    }
    
    public function getColores(){
        $color = R::getAll('select * from color');
        return $color;
    }

    public function getComidas(){
        $comida = R::getAll('select * from comida');
        return $comida;
    }

    public function getMusicas(){
        $musica = R::getAll('select * from musica');
        return $musica;
    }

    public function getPeliculas(){
        $pelicula = R::getAll('select * from pelicula');
        return $pelicula;
    }
    ...
    public function grabarExcel(){
        ...
        if($aprobado){
            ...
            for ($row = 2; $row <= $highestRow; $row++){
                ...
                $orden = R::dispense('perfil'); //Entre comillas vá tu tabla
                $orden->nombre = $columna0; //Ahí en nombre es como se llama tu campo en la tabla y qué le vás a meter
                $orden->apellido = $columna1;           

                // Color
                $color = $sheet->getCellByColumnAndRow(2,$row)->getCalculatedValue();     
                ...
                $comida = $sheet->getCellByColumnAndRow(3,$row)->getCalculatedValue();   
                //$comida = (int)$comida;
                ...
                $musica = $sheet->getCellByColumnAndRow(4,$row)->getCalculatedValue();   
                //$musica = (int)$musica;  

                $pelicula = $sheet->getCellByColumnAndRow(5,$row)->getCalculatedValue();   
                //$pelicula = (int)$pelicula;

                $esSoltero = $sheet->getCellByColumnAndRow(6,$row)->getCalculatedValue();   
                ...
                $esDeportista = $sheet->getCellByColumnAndRow(7,$row)->getCalculatedValue();   
                ...
                $orden->color = $resultadoColor;
                $orden->comida = $resultadoComida;
                $orden->musica = $musica;
                $orden->pelicula = $pelicula;
                $orden->esSoltero = $esSoltero;
                $orden->esDeportista = $esDeportista;  
                $orden->esVegetariano = $esVegetariano;                  
