...
<script>
    $(function(){
        ...
        $("#ventanaMoneda").jqxWindow({showCollapseButton: false, height: 100, width: 350, theme: theme,
        resizable: false, keyboardCloseKey: -1});
        if ($("#id").val() == 0){
            $("#titulo").text('Nuevo Moneda');
        } else {
            $("#titulo").text('Editar Moneda');
            datos = {
                id: $("#id").val()
            };
            $.post('/moneda/getMoneda', datos, function(data){
                $("#nombre").val(data.nombre);
            }
            , 'json');
        };
         $('#form').jqxValidator({ rules: [
                    { input: '#nombre', message: 'Debe ingresar el nombre del moneda!',  rule: 'required' },
                    { input: '#nombre', message: 'Ya existe un moneda con ese nombre!',  rule: function(){
                            datos = {
                                tabla: 'moneda',
                                campo: 'nombre',
                                valor: $('#nombre').val(),
        ...
        $('#aceptarButton').bind('click', function () {
            ...
            if (formOK){
                $('#ventanaMoneda').ajaxloader();
                datos = {
                    id: $("#id").val(),
                    nombre: $('#nombre').val()
                }
                $.post('/moneda/saveMoneda', datos, function(data){
                    if (data.id > 0){
                        $.redirect('/moneda');
                    } else {
                        new Messi('Hubo un error guardando el moneda', {title: 'Error', 
                            buttons: [{id: 0, label: 'Cerrar', val: 'X'}], modal:true});
                        $('#ventanaMoneda').ajaxloader('hide');
                    }
                }, 'json');
            }
        });                
